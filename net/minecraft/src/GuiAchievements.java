...
public class GuiAchievements extends GuiScreen
{
    ...
    protected void genAchievementBackground(int par1, int par2, float par3)
    {
        ...
        if (var30 != null)
        {
            Achievement achievement2 = var30;
        
        if (CheatPack.isEnabled(mc.theWorld.isRemote))
        {
            if (Mouse.isButtonDown(0) && !statFileWriter.hasAchievementUnlocked(achievement2))
            {
                CheatPack.sspBase.unlockAchievement(mc, achievement2);
            }
            else if (Mouse.isButtonDown(1) && statFileWriter.hasAchievementUnlocked(achievement2))
            {
                CheatPack.sspBase.lockAchievement(mc.statFileWriter, achievement2);
            }
        }
